define([
	...
	function( Backbone ) {
		...
		return Backbone.Model.extend({
			...
			save: function() {
				var savedEpochGroups;
				if(!localStorage['epochGroups']) {
					savedEpochGroups = [];
				}
				else {
					savedEpochGroups = JSON.parse(localStorage['epochGroups']);
				}
				savedEpochGroups.push(this.toJSON());
				localStorage['epochGroups'] = JSON.stringify(savedEpochGroups);
			},
